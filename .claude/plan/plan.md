...
| TLS | rustls (aws-lc-rs) everywhere; **TLS 1.3 is required for ATAK on :8089** (its `DEFAULT:!ECDH` cipher string removes every rustls TLS 1.2 suite); 1.2 kept for CloudTAK/WinTAK/iTAK. CI runs `openssl s_client -cipher 'DEFAULT:!ECDH'` against the stream port; escape hatch (unplanned) is an OpenSSL acceptor feature for :8089 only. Root CA and internal server certs RSA-2048 by default (ecdsa-p256 selectable); client certs get the CSR's key but **our** subject (`CN=<user>` + configured O/OU), EKU clientAuth (+ optional TAK channels marker OID), 128-bit serial, SKI/AKI. Revocation enforced in a custom rustls `ClientCertVerifier` via an in-memory cache; `require_known_cert` rejects CA-signed certs missing from the DB. |
...
- **Interop suites in CI** (`interop/`):
  ...
  - `interop/eud/` — an ATAK-side harness. Preferred candidate: build ATAK's networking core `commoncommo` (C++) from the atak-civ source at CI time inside a Docker image (a GPL test tool, not linked into rustak) and drive its example client for enrollment, TLS 1.3 + `!ECDH` handshake, protobuf negotiation, SA/chat routing and mission-package transfer; fallback candidates: an Android emulator job with ATAK-CIV, or a pytak/`takproto` client. **An exploration brief in M1 decides** (feasibility, build time, licensing hygiene) before it is built; nightly once it exists.
...
- Quality gates in CI: `cargo fmt --check`, `cargo clippy --all-targets -D warnings`, `cargo doc -D warnings`, `cargo audit`, file-length script, `openssl s_client -cipher 'DEFAULT:!ECDH'` TLS 1.3 check against :8089.
