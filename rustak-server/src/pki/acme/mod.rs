...
//! [`Services`](crate::services::Services). Both of them reach it through
//! [`AcmeState`], which the context carries: a process-wide static would be
//! one listener's resolver answering for another server's renewal, which is
//! exactly what two tests in one process are. Nothing reads it except the
//! renewal, the challenge routes and the admin API, and all of them treat its
//! absence — a listener that is not in ACME mode — as "no certificate can be
//! swapped in", not as a failure.
...
pub use renew::{CertState, Decision, backoff, decide, next_renewal, run, status};
pub use state::AcmeState;
