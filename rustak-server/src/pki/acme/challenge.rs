...
//!
//! # Why the token map is a handle rather than a static
//!
//! actix builds one `App` per worker thread, and the handler has nothing but
//! its request and whatever the application was configured with: the challenge
//! is published by a background job, so the two have to share something.
//! [`AcmeState`] is that something — one per [`AppContext`](crate::services::AppContext),
//! handed to [`routes`] where the listener is built and read by the renewal
//! through the services it already holds. It was a process-wide static, which
//! made every server in a process answer for every other one's order; two
//! integration tests running concurrently are exactly that.
//!
//! The map is empty except during the seconds an authority is being answered,
//! because [`AcmeState::withdraw`] runs as soon as validation finishes.
...
use crate::pki::tls::TlsAlpnChallenge;

use super::state::AcmeState;
...
/// exist. Mounted on the public listener and on the plaintext one
/// ([`web::plain`](crate::web::plain)), which is the port an authority
/// validating `http-01` actually connects to.
pub fn routes(state: Arc<AcmeState>) -> impl FnOnce(&mut web::ServiceConfig) + Clone {
    move |config| {
        config.app_data(web::Data::from(Arc::clone(&state))).route(
...
async fn respond(token: web::Path<String>, state: web::Data<AcmeState>) -> HttpResponse {
    match state.answer(&token) {
...
///
/// Holds the installation's [`AcmeState`], which carries the listener's
/// certificate resolver when there is one — and that is what decides whether
/// `tls-alpn-01` can be answered at all, because a resolver is only published
/// for `[web.public.tls] mode = "acme"`.
pub struct Responder {
    state: Arc<AcmeState>,
}

impl Responder {
    /// A responder that answers `tls-alpn-01` through `state`'s resolver when
    /// it has one, and `http-01` through `state`'s token map either way.
    ...
    pub fn can_answer(&self, kind: AcmeChallenge) -> bool {
        match kind {
            AcmeChallenge::TlsAlpn01 => self.state.resolver().is_some(),
    ...
    pub fn publish(
        ...
        match kind {
            AcmeChallenge::Http01 => {
                self.state.publish(token, key_authorization);

                Ok(())
            }
            AcmeChallenge::TlsAlpn01 => {
                let resolver = self.state.resolver().ok_or_else(|| {
                    human_errors::system(
    ...
    pub fn withdraw(&self, kind: AcmeChallenge, token: &str) {
        match kind {
            AcmeChallenge::Http01 => self.state.withdraw(token),
            AcmeChallenge::TlsAlpn01 => {
...
mod tests {
    ...
    /// A handle with a resolver on it, as `mode = "acme"` publishes.
    fn with_resolver() -> (Arc<AcmeState>, Arc<HotSwapCertResolver>) {
    ...
    async fn a_token_nobody_published_is_not_found() {
        let app =
            test::init_service(App::new().configure(routes(Arc::new(AcmeState::new())))).await;
    ...
    async fn a_published_token_is_answered_with_exactly_the_key_authorization() {
        ...
        let state = Arc::new(AcmeState::new());
        state.publish(token, &authorization(token));
