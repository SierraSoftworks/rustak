...
/// The `issued_via` value a certificate bought with a workload identity
/// carries, and the one `revoke_previous` matches on.
...
pub enum IssuedVia {
    ...
    /// A sidecar presented its orchestrator's workload identity — a Nomad
    /// `identity` JWT or a Kubernetes projected service-account token —
    /// instead of a one-time enrolment token.
    ///
    /// Recorded as its own value rather than folded into the two `signClient`
    /// ones because `[auth.workload] revoke_previous` matches on it: the
    /// certificates an account is holding *from a workload identity* are the
    /// ones a new one supersedes, and nothing else it holds is touched.
    WorkloadIdentity,
...
mod tests {
    ...
    fn every_endpoint_has_a_stored_name_the_schema_accepts() {
        for via in [
            ...
            IssuedVia::WorkloadIdentity,
        ] {
            assert!(!via.as_str().is_empty());
        }

        assert_eq!(IssuedVia::WorkloadIdentity.as_str(), WORKLOAD_IDENTITY);
        assert_eq!(
            IssuedVia::WorkloadIdentity.source(),
            CertificateSource::Enrollment,
            "the key was generated by the client, exactly as for an enrolment token",
        );
