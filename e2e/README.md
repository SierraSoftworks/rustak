...
| `map.spec.ts` | The map, against the demo fixtures and with every tile request refused: the libraries load from `/vendor`, what the API returns reaches the map, the roster searches by callsign and type, picking a row opens its pop-over, a click on two things at once opens the chooser, and a marker's type and symbol are chosen from the catalogues by walking or searching them, each row with a picture of its symbol. |
| `drawing.spec.ts` | Drawing on the map, against the demo fixtures: a line, a polygon, a rectangle, a circle and a route are each drawn with clicks and appear as one; a polygon is coloured, reshaped by dragging a corner and deleted; a click is taken back and a drawing given up; a shape from elsewhere is editable and a route from elsewhere is not; and on a phone the tools fit and the list folds out of the way. |
