...
| `map.spec.ts` | The map, against the demo fixtures and with every tile request refused: the libraries load from `/vendor`, what the API returns reaches the map, the roster searches by callsign and type, and picking a row opens its pop-over. |
