...
//!
//! The sentence is written here rather than on [`ApiError::Gone`] itself. The
//! same status now also means "that mission was deleted", and one message
//! cannot be right for both; the call site is what knows which question it
//! asked.
...
/// What a `410` from any of these routes means.
///
/// The wizard is one linear walk and every step of it is gone at the same
/// moment, so the sentence is the same for all of them.
fn completed(err: ApiError) -> ApiError {
    match err {
        ApiError::Gone => ApiError::Server("This server has already been set up.".to_string()),
        other => other,
    }
}
...
pub async fn create_admin(request: &CreateAdminRequest) -> Result<AdminCreated, ApiError> {
    ...
    post_json("/setup/admin", request).await.map_err(completed)
}
...
pub async fn set_server(request: &ServerSettingsRequest) -> Result<ServerSettings, ApiError> {
    ...
    post_json("/setup/server", request).await.map_err(completed)
}
...
pub async fn ca() -> Result<Option<CaSummary>, ApiError> {
    ...
    json_response(response).await.map(Some).map_err(completed)
}
...
pub async fn init_ca(request: &InitCaRequest) -> Result<CaSummary, ApiError> {
    ...
    post_json("/setup/ca", request).await.map_err(completed)
}
...
pub async fn complete() -> Result<(), ApiError> {
    ...
    post_empty("/setup/complete", &serde_json::json!({}))
        .await
        .map_err(completed)
}
